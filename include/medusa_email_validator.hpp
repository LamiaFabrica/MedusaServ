...
private:
    ...
    int cache_ttl_ = 3600;
    bool yorkshire_champion_mode_ = true;
    std::string txt_resolver_name_;  // nameserver for TXT queries
    std::string mx_resolver_name_;   // nameserver for MX queries
    std::string a_resolver_name_;    // nameserver for A queries
    std::function<std::vector<DNSRecord>(const std::string&)> txt_resolver_;
    std::function<std::vector<DNSRecord>(const std::string&)> mx_resolver_;
    std::function<std::vector<DNSRecord>(const std::string&)> a_resolver_;
...
public:
    ...
    void set_yorkshire_champion_mode(bool enabled) { yorkshire_champion_mode_ = enabled; }
    // Override the nameserver (tests / custom resolvers). "" reverts to the
    // /etc/resolv.conf default.
    void set_nameserver(const std::string& nameserver);
    // Inject a record resolver (tests use this instead of the live DNS).
    void set_txt_resolver(std::function<std::vector<DNSRecord>(const std::string&)> fn) { txt_resolver_ = std::move(fn); }
    void set_mx_resolver(std::function<std::vector<DNSRecord>(const std::string&)> fn) { mx_resolver_ = std::move(fn); }
    void set_a_resolver(std::function<std::vector<DNSRecord>(const std::string&)> fn) { a_resolver_ = std::move(fn); }
};
